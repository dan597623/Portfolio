...
try {
  ...
  await page.locator('.case-study__link[href="/projects/xunlocked"]').click();
  await page.waitForURL('**/projects/xunlocked');
  await page.waitForLoadState('networkidle');
  assert.match(await page.locator('h1').textContent(), /xUnlocked:/);
  assert.equal(await page.locator('.post-index nav a').count(), 7);
  await page.screenshot({ path: 'test-results/xunlocked-desktop.png' });
  ...
  await page.goto('http://127.0.0.1:5173/projects/xunlocked', { waitUntil: 'networkidle' });
  await page.screenshot({ path: 'test-results/xunlocked-mobile.png' });
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
