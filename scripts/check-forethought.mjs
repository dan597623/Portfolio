...
try {
  ...
  await page.locator('.case-study__link[href="/projects/forethought"]').evaluate(el => el.closest('.template-carousel').querySelector('.swiper-button-next').click());
  await page.locator('.case-study__link[href="/projects/forethought"]').click();
  await page.waitForURL('**/projects/forethought');
  await page.waitForLoadState('networkidle');
  assert.match(await page.locator('h1').textContent(), /Forethought:/);
  assert.equal(await page.locator('.post-index nav a').count(), 7);
  await page.screenshot({ path: 'test-results/forethought-desktop.png' });
  ...
  await page.goto('http://127.0.0.1:5173/projects/forethought', { waitUntil: 'networkidle' });
  await page.screenshot({ path: 'test-results/forethought-mobile.png' });
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
