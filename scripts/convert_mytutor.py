...
source = Path(r'C:\Users\aurora\Downloads\Company logo\MyTutor online tutoring platform for schools.html')
assets = source.with_name(source.stem + '_files')
soup = BeautifulSoup(source.read_text(encoding='utf-8'), 'html.parser')
destination = root / 'public/assets/mytutor'
...
downloads = {}
def local(url):
    if '_files/' in url: return '/assets/mytutor/' + url.split('_files/')[-1]
...
output = root/'src/components/mytutor'
