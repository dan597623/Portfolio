...
source = Path(r'C:\Users\aurora\Downloads\Company logo\Levels Health — a mobile app for tracking metabolic health.html')
assets = source.with_name(source.stem + '_files')
soup = BeautifulSoup(source.read_text(encoding='utf-8'), 'html.parser')
destination = root / 'public/assets/levels-health'
...
downloads = {}
def local(url):
    if '_files/' in url: return '/assets/levels-health/' + url.split('_files/')[-1]
...
output = root/'src/components/levels-health'
