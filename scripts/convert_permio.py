...
source = Path(r'C:\Users\aurora\Downloads\Company logo\AI-driven permit automation for Service First Permits.html')
assets = source.with_name(source.stem + '_files')
soup = BeautifulSoup(source.read_text(encoding='utf-8'), 'html.parser')
destination = root / 'public/assets/permio'
...
downloads = {}
def local(url):
    if '_files/' in url: return '/assets/permio/' + url.split('_files/')[-1]
...
output = root/'src/components/permio'
