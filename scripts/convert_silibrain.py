...
source = Path(r'C:\Users\aurora\Downloads\Company logo\Silibrain_ Digital transformation of healthcare systems.html')
assets = source.with_name(source.stem + '_files')
soup = BeautifulSoup(source.read_text(encoding='utf-8'), 'html.parser')
destination = root / 'public/assets/silibrain'
...
downloads = {}
def local(url):
    if '_files/' in url: return '/assets/silibrain/' + url.split('_files/')[-1]
...
output = root/'src/components/silibrain'
