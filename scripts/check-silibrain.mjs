...
try {
  ...
  await page.locator('.case-study__link[href="/projects/silibrain"]').evaluate(el => { const swiper = el.closest('.swiper').swiper; swiper.slideTo(swiper.slides.length - 1, 0); });
  await page.locator('.case-study__link[href="/projects/silibrain"]').click();
  await page.waitForURL('**/projects/silibrain');
  await page.waitForLoadState('networkidle');
  assert.match(await page.locator('h1').textContent(), /Silibrain:/);
  assert.equal(await page.locator('.post-index nav a').count(), 7);
  await page.screenshot({ path: 'test-results/silibrain-desktop.png' });
  ...
  await page.goto('http://127.0.0.1:5173/projects/silibrain', { waitUntil: 'networkidle' });
  await page.screenshot({ path: 'test-results/silibrain-mobile.png' });
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
