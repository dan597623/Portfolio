...
try {
  ...
  await page.locator('.case-study__link[href="/projects/levels-health"]').click();
  await page.waitForURL('**/projects/levels-health');
  await page.waitForLoadState('networkidle');
  assert.match(await page.locator('h1').textContent(), /Levels Health/i);
  assert.equal(await page.locator('.post-index nav a').count(), 5);
  await page.screenshot({ path: 'test-results/levels-health-desktop.png' });
  ...
  await page.goto('http://127.0.0.1:5173/projects/levels-health', { waitUntil: 'networkidle' });
  await page.screenshot({ path: 'test-results/levels-health-mobile.png' });
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
  ...
  console.log('PASS: portfolio link, direct route, five contents links, related carousel, theme, mobile menu, responsive widths, and all images; no browser errors.');
} finally {
  await browser.close();
}
