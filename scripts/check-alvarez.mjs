...
try {
  ...
  await page.locator('.case-study__thumb-link[href="/projects/alvarez-and-marsal"]').click();
  await page.waitForURL('**/projects/alvarez-and-marsal');
  await page.waitForLoadState('networkidle');
  assert.match(await page.locator('h1').textContent(), /A&M/);
  assert.equal(await page.locator('.post-index nav a').count(), 7);
  await page.screenshot({ path: 'test-results/alvarez-desktop.png' });
  ...
  await page.goto('http://127.0.0.1:5173/projects/alvarez-and-marsal', { waitUntil: 'networkidle' });
  await page.screenshot({ path: 'test-results/alvarez-mobile.png' });
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
