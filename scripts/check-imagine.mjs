...
try {
  ...
  await page.locator('.case-study__thumb-link[href="/projects/imagine-ai"]').click();
  await page.waitForURL('**/projects/imagine-ai');
  await page.waitForLoadState('networkidle');
  assert.match(await page.locator('h1').textContent(), /Imagine AI/);
  assert.equal(await page.locator('.post-index nav a').count(), 7);
  await page.screenshot({ path: 'test-results/imagine-desktop.png' });
  ...
  await page.goto('http://127.0.0.1:5173/projects/imagine-ai', { waitUntil: 'networkidle' });
  await page.screenshot({ path: 'test-results/imagine-mobile.png' });
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
