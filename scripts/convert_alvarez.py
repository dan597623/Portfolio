...
source = Path(r'C:\Users\aurora\Downloads\Company logo\How we streamlined enterprise logistics for A&M.html')
assets = source.with_name(source.stem + '_files')
soup = BeautifulSoup(source.read_text(encoding='utf-8'), 'html.parser')
destination = root / 'public/assets/alvarez-and-marsal'
...
downloads = {}
def local(url):
    if '_files/' in url: return '/assets/alvarez-and-marsal/' + url.split('_files/')[-1]
...
output = root/'src/components/alvarez'
