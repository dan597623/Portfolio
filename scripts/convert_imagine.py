...
source = Path(r'C:\Users\aurora\Downloads\Company logo\Imagine AI_ Accelerating hiring by up to 90%.html')
assets = source.with_name(source.stem + '_files')
soup = BeautifulSoup(source.read_text(encoding='utf-8'), 'html.parser')
destination = root / 'public/assets/imagine-ai'
...
downloads = {}
def local(url):
    if '_files/' in url: return '/assets/imagine-ai/' + url.split('_files/')[-1]
...
output = root/'src/components/imagine'
