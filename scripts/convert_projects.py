...
source = Path(r'C:\Users\aurora\Downloads\Company logo\Our best works by industries and services _ Geniusee.html')
assets = source.with_name(source.stem + '_files')
soup = BeautifulSoup(source.read_text(encoding='utf-8'), 'html.parser')
destination = root / 'public/assets/projects'
...
downloads = {}
def local(url):
    if '_files/' in url: return '/assets/projects/' + url.split('_files/')[-1]
...
for node in soup.select('.posts-filter-tags-list, .posts-filter-clear, .posts-filter__more'):
    node.decompose()
output = root/'src/components/projects'
output.mkdir(parents=True,exist_ok=True)
for name, node in zip(['ProjectsHero','LatestProjects','ProjectGrid','AboutCompany','Testimonials','ProjectReviews'], sections[:6]):
    content = jsx(node)
    (output/f'{name}.jsx').write_text(f'export default function {name}() {{\n return (\n{content}\n );\n}}\n',encoding='utf-8')
