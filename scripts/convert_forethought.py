...
source = Path(r'C:\Users\aurora\Downloads\Company logo\Enterprise AI customer support integration for Forethought.html')
assets = source.with_name(source.stem + '_files')
soup = BeautifulSoup(source.read_text(encoding='utf-8'), 'html.parser')
destination = root / 'public/assets/forethought'
...
downloads = {}
def local(url):
    if '_files/' in url: return '/assets/forethought/' + url.split('_files/')[-1]
...
output = root/'src/components/forethought'
