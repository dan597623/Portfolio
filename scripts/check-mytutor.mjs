...
try {
  ...
  await page.locator('.case-study__thumb-link[href="/projects/my-tutor"]').click();
  await page.waitForURL('**/projects/my-tutor');
  await page.waitForLoadState('networkidle');
  assert.match(await page.locator('h1').textContent(), /MyTutor/);
  assert.equal(await page.locator('.post-index nav a').count(), 6);
  await page.screenshot({ path: 'test-results/mytutor-desktop.png' });
  ...
  await page.goto('http://127.0.0.1:5173/projects/my-tutor', { waitUntil: 'networkidle' });
  await page.screenshot({ path: 'test-results/mytutor-mobile.png' });
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
