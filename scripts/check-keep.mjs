...
try {
  ...
  await page.locator('.case-study__link[href="/projects/keep"]').evaluate(el => el.closest('.template-carousel').querySelector('.swiper-button-next').click());
  await page.waitForTimeout(400);
  await page.locator('.case-study__link[href="/projects/keep"]').evaluate(el => el.closest('.template-carousel').querySelector('.swiper-button-next').click());
  await page.locator('.case-study__link[href="/projects/keep"]').click();
  await page.waitForURL('**/projects/keep');
  await page.waitForLoadState('networkidle');
  assert.match(await page.locator('h1').textContent(), /Keep:/);
  assert.equal(await page.locator('.post-index nav a').count(), 6);
  await page.screenshot({ path: 'test-results/keep-desktop.png' });
  ...
  await page.goto('http://127.0.0.1:5173/projects/keep', { waitUntil: 'networkidle' });
  await page.screenshot({ path: 'test-results/keep-mobile.png' });
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
  ...
  console.log('PASS: homepage link, direct route, six contents links, related carousel, theme, mobile menu, responsive widths, and all images; no browser errors.');
} finally {
  await browser.close();
}
