...
source = Path(r'C:\Users\aurora\Downloads\Company logo\xUnlocked multi-domain learning platform development.html')
assets = source.with_name(source.stem + '_files')
soup = BeautifulSoup(source.read_text(encoding='utf-8'), 'html.parser')
destination = root / 'public/assets/xunlocked'
...
downloads = {}
def local(url):
    if '_files/' in url: return '/assets/xunlocked/' + url.split('_files/')[-1]
...
output = root/'src/components/xunlocked'
