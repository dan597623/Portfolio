...
try {
  ...
  await page.locator('.case-study__link[href="/projects/permio"]').click();
  await page.waitForURL('**/projects/permio');
  await page.waitForLoadState('networkidle');
  assert.match(await page.locator('h1').textContent(), /Permio:/);
  assert.equal(await page.locator('.post-index nav a').count(), 8);
  await page.screenshot({ path: 'test-results/permio-desktop.png' });
  ...
  await page.goto('http://127.0.0.1:5173/projects/permio', { waitUntil: 'networkidle' });
  await page.screenshot({ path: 'test-results/permio-mobile.png' });
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
  ...
  console.log('PASS: homepage link, direct route, eight contents links, related carousel, theme, mobile menu, responsive widths, and all images; no browser errors.');
} finally {
  await browser.close();
}
