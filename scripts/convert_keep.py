...
source = Path(r'C:\Users\aurora\Downloads\Company logo\Keep_ FinTech-powered solution for employee retention.html')
assets = source.with_name(source.stem + '_files')
soup = BeautifulSoup(source.read_text(encoding='utf-8'), 'html.parser')
destination = root / 'public/assets/keep'
...
downloads = {}
def local(url):
    if '_files/' in url: return '/assets/keep/' + url.split('_files/')[-1]
...
output = root/'src/components/keep'
